...
def get_features(features,name):
    def hook(model, input, output):
        features['output'] = output[0] if isinstance(output,tuple) else output
    return hook
###################################


class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, augment_pipe=None, style_mixing_prob=0.9, 
                        r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2, with_dataaug=False,
                        lamd=0.0, reg_model=None,layer_index=None,mean=None,std=None):
        super().__init__()
        self.device = device
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                ######################################################
                loss_reg = 0.0
                real_img_tmp_reg = real_img.detach()
                if real_img_tmp_reg.shape[-1]==32:
                    output1 = self.reg_model(torch.nn.functional.interpolate((real_img_tmp_reg - self.mean)/self.std, scale_factor=8)).detach()
                    output2 = self.reg_model(torch.nn.functional.interpolate((gen_img - self.mean)/self.std, scale_factor=8))
                else:
                    output1 = self.reg_model((real_img_tmp_reg - self.mean)/self.std).detach()
                    output2 = self.reg_model((gen_img - self.mean)/self.std)
                loss_reg = torch.nn.functional.mse_loss(input=output2, target=output1)
                training_stats.report('Loss/G/pretrained_model_loss', loss_reg)
                ######################################################
                ######################################################
                total_G_loss = loss_Gmain + self.lamd * loss_reg
